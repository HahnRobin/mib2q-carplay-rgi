...
([[supervisor-lifecycle]]). Plane routing and geometry: [[display-contexts]], [[kdk-geometry]].

## Context

> `BAPBridge` -> `RendererMapper` -> `RendererServer` :19800 -> **maneuver-renderer** -> displayable 98
> -> [[compositing]]. Progress: [[bargraph-sync]]. Lanes: [[lane-guidance]].
...
| 0x06 | `CMD_PROGRESS` | `[0]` remaining level 0-16, `[1]` mode, `[2]` state (flag 0x20) - [[bargraph-sync]] |
...
| 0x0c-0x0e | `CMD_LANES_BEGIN/LANE/COMMIT` | atomic lane batch - [[lane-guidance]] |
...
This gates presentation replay, not the context ([[rgd-activation]]).

## What Java sends (`RendererMapper`)

The BAP family from [[maneuver-mapping]] plus raw CarPlay geometry, all as half-degrees:
