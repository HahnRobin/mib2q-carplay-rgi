/*
 * Single-thread TCP CLIENT — connects to Java BAPBridge server
 * (127.0.0.1:19800), receives CMD_MANEUVER / CMD_PROGRESS / etc.
 *
 * No pthreads — heartbeat is dispatched by the main render loop on
 * its 1 s tick.  Net effect: renderer runs in a single thread, which
 * removes scheduler interactions with the QNX 6.5 kernel TCP stack
 * that were causing periodic ~245 ms freezes in the prior threaded
 * implementation.
 *
 * Topology: renderer is the short-lived process spawned per-route.
 * Java (long-lived) accepts whenever we connect.  No timing/sleep
 * hacks needed.
 *
 * POSIX sockets — works on macOS and QNX 6.5.
 *
 * Copyright (c) 2026 LuKa (@LuKa_dev)
 */

#include <stdio.h>
#include <string.h>
#include <unistd.h>
#include <errno.h>
#include <fcntl.h>
#include <sys/types.h>
#include <sys/time.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <netinet/tcp.h>
#include <arpa/inet.h>

#include "server.h"

static int g_server_fd = -1;        /* connected fd to Java BAPBridge */
static int g_target_port = 0;
static struct timeval g_last_retry = {0, 0};

#define RECV_BUF_SIZE 512
static uint8_t g_recv_buf[RECV_BUF_SIZE];
static int g_recv_len = 0;
static int g_ready_sent = 0;
static int g_frame_ready_sent = 0;

/* Renderer->Java events are fixed-size packets, but the socket is deliberately
 * non-blocking. Preserve their order across a transient EAGAIN so the
 * FRAME_CLEARED barrier cannot disappear between an old and a new FRAME_READY. */
#define EVENT_QUEUE_SIZE 16
static uint8_t g_event_queue[EVENT_QUEUE_SIZE];
static int g_event_head = 0;
static int g_event_count = 0;

#define RETRY_INTERVAL_SEC 1   /* re-attempt connect every 1 s if Java not up yet */

/* Any lost peer invalidates the visible frame, including RST/write failure.
 * Main clears content before accepting fresh state on the reconnected link. */
static int g_peer_closed = 0;

int cr_server_peer_closed(void) {
    return g_peer_closed;
}

/* Reset the peer-closed latch after the always-on renderer has handled a disconnect
 * (cleared its content) and wants to keep running + reconnect. */
void cr_server_clear_peer_closed(void) {
    g_peer_closed = 0;
}

static void set_nonblocking(int fd) {
    int flags = fcntl(fd, F_GETFL, 0);
    if (flags >= 0)
        fcntl(fd, F_SETFL, flags | O_NONBLOCK);
}

static void clear_event_queue(void) {
    g_event_head = 0;
    g_event_count = 0;
}

static void drop_peer(const char *reason) {
    if (reason) fprintf(stderr, "server: peer reset: %s\n", reason);
    if (g_server_fd >= 0) close(g_server_fd);
    g_server_fd = -1;
    g_recv_len = 0;
    g_peer_closed = 1;
    g_frame_ready_sent = 0;
    clear_event_queue();
}

static void flush_event_queue(void) {
    while (g_server_fd >= 0 && g_event_count > 0) {
        uint8_t pkt[CR_PKT_SIZE];
        ssize_t k;
        memset(pkt, 0, sizeof(pkt));
        pkt[0] = g_event_queue[g_event_head];
        k = send(g_server_fd, pkt, CR_PKT_SIZE, 0);
        if (k == (ssize_t)CR_PKT_SIZE) {
            g_event_head = (g_event_head + 1) % EVENT_QUEUE_SIZE;
            g_event_count--;
            continue;
        }
        if (k < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) return;

        /* A short TCP write cannot be resumed as a fresh 48-byte protocol
         * packet. Reset the connection: Java clears frameReady/clearPending on
         * disconnect, then READY state is replayed on reconnect. */
        drop_peer(k >= 0 ? "short event write" : strerror(errno));
        return;
    }
}

static void send_event(uint8_t event_id) {
    int tail;
    if (g_server_fd < 0) return;

    if (g_event_count >= EVENT_QUEUE_SIZE) {
        /* Never discard a barrier while pretending the connection stayed
         * valid. A reconnect gives Java a clean, explicitly-not-ready state. */
        drop_peer("event queue full");
        return;
    }

    tail = (g_event_head + g_event_count) % EVENT_QUEUE_SIZE;
    g_event_queue[tail] = event_id;
    g_event_count++;
    flush_event_queue();
}

static int try_connect(void) {
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) return -1;

    int one = 1;
    setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &one, sizeof(one));

    /* Non-blocking connect with a 300ms ceiling.  Without this a stale
     * SYN to a port whose listener is dead-but-not-yet-RST could pin
     * the render loop for the kernel TCP SYN timeout (multiple seconds
     * on QNX 6.5).  We stay loopback-only so 300ms is generous. */
    set_nonblocking(fd);

    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    addr.sin_port = htons((uint16_t)g_target_port);

    int rc = connect(fd, (struct sockaddr *)&addr, sizeof(addr));
    if (rc < 0 && errno != EINPROGRESS && errno != EWOULDBLOCK) {
        close(fd);
        return -1;
    }

    if (rc < 0) {
        fd_set wfds;
        FD_ZERO(&wfds);
        FD_SET(fd, &wfds);
        struct timeval tv = { .tv_sec = 0, .tv_usec = 300 * 1000 };
        int sel = select(fd + 1, NULL, &wfds, NULL, &tv);
        if (sel <= 0) {
            close(fd);
            return -1;
        }
        int soerr = 0;
        socklen_t slen = sizeof(soerr);
        if (getsockopt(fd, SOL_SOCKET, SO_ERROR, &soerr, &slen) < 0 || soerr != 0) {
            close(fd);
            return -1;
        }
    }

    return fd;
}

int cr_server_init(int port) {
    g_target_port = port;
    clear_event_queue();

    /* First connect attempt — Java should already be listening. */
    g_server_fd = try_connect();
    if (g_server_fd >= 0) {
        fprintf(stderr, "server: connected to 127.0.0.1:%d\n", port);
    } else {
        fprintf(stderr, "server: 127.0.0.1:%d not yet listening, will retry\n", port);
    }
    g_recv_len = 0;
    return 0;
}

void cr_server_poll(void) {
    /* Reconnect if disconnected, throttled to RETRY_INTERVAL_SEC. */
    if (g_server_fd < 0 && g_target_port > 0) {
        struct timeval now;
        gettimeofday(&now, NULL);
        if (now.tv_sec - g_last_retry.tv_sec >= RETRY_INTERVAL_SEC) {
            g_last_retry = now;
            g_server_fd = try_connect();
            if (g_server_fd >= 0) {
                fprintf(stderr, "server: connected to 127.0.0.1:%d\n", g_target_port);
                g_recv_len = 0;
                clear_event_queue();
                if (g_ready_sent) send_event(EVT_READY);
                if (g_frame_ready_sent) send_event(EVT_FRAME_READY);
            }
        }
    }

    /* Retry ordered events that previously hit EAGAIN before processing the
     * next command batch. */
    flush_event_queue();

    /* Read available data from server */
    if (g_server_fd >= 0) {
        int space = RECV_BUF_SIZE - g_recv_len;
        if (space > 0) {
            int n = (int)recv(g_server_fd, g_recv_buf + g_recv_len, space, 0);
            if (n > 0) {
                g_recv_len += n;
            } else if (n == 0) {
                drop_peer("peer closed");
            } else if (errno != EAGAIN && errno != EWOULDBLOCK && errno != EINTR) {
                drop_peer(strerror(errno));
            }
        }
    }
}

int cr_server_read_cmd(cr_cmd_t *out) {
    if (g_recv_len < CR_PKT_SIZE) return 0;

    memcpy(out, g_recv_buf, CR_PKT_SIZE);
    g_recv_len -= CR_PKT_SIZE;
    if (g_recv_len > 0)
        memmove(g_recv_buf, g_recv_buf + CR_PKT_SIZE, g_recv_len);
    return 1;
}

/* Send EVT_HEARTBEAT — single 48-byte packet, all-zero payload, cmd=0x80.
 * Called by main loop on its 1 s tick.  Non-blocking; on failure we just
 * skip — main thread's cr_server_poll() will detect a dead socket via
 * recv error on the next iteration. */
void cr_server_send_heartbeat(void) {
    send_event(EVT_HEARTBEAT);
}

void cr_server_mark_ready(void) {
    g_ready_sent = 1;
    send_event(EVT_READY);
}

void cr_server_mark_frame_ready(void) {
    g_frame_ready_sent = 1;
    send_event(EVT_FRAME_READY);
}

void cr_server_clear_frame_ready(void) {
    g_frame_ready_sent = 0;
}

void cr_server_mark_frame_cleared(void) {
    send_event(EVT_FRAME_CLEARED);
}
void cr_server_shutdown(void) {
    if (g_server_fd >= 0) { close(g_server_fd); g_server_fd = -1; }
    g_recv_len = 0;
    clear_event_queue();
    fprintf(stderr, "server: shutdown\n");
}
